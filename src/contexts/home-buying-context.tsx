...
export interface HomeBuyingData {
  ...
  minBudget: number
  currency: string
  propertyType: 'single-family' | 'townhouse' | 'condo' | 'multi-family' | 'any'
  bedrooms: number
  bathrooms: number
  ...
  searchTimeline: '1-month' | '3-months' | '6-months' | '12-months' | 'flexible'
  firstTimeBuyer: boolean
  currentLivingSituation: 'renting' | 'living-with-family' | 'own-home' | 'other'
...
export function HomeBuyingProvider({ children }: { children: ReactNode }) {
  const [homeBuyingData, setHomeBuyingData] = useState<HomeBuyingData>(defaultHomeBuyingData)
  ...
  useEffect(() => {
    localStorage.setItem('templata-home-buying-data', JSON.stringify(homeBuyingData))
  }, [homeBuyingData])
  ...
  return (
    <HomeBuyingContext.Provider value={{
      homeBuyingData,
      isWizardOpen,
      setWizardOpen,
...
export function getHomeBuyingDisplayData(data: HomeBuyingData) {
  ...
  const affordabilityRatio = (monthlyPayment / data.monthlyIncomeAfterTax) * 100
  
  return {
    targetLocation: data.targetLocation || "Your Target Area",
    budgetRange: `${data.currency} ${data.minBudget.toLocaleString()} - ${data.maxBudget.toLocaleString()}`,
    propertyType: data.propertyType.charAt(0).toUpperCase() + data.propertyType.slice(1).replace('-', ' '),
    ...
    searchTimeline: data.searchTimeline.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()),
    monthlyPayment: Math.round(monthlyPayment),
    affordabilityRatio: Math.round(affordabilityRatio),
    downPaymentDisplay: `${data.currency} ${data.downPaymentAmount.toLocaleString()} (${data.downPaymentPercentage}%)`,
    firstTimeBuyer: data.firstTimeBuyer ? "First-time buyer" : "Experienced buyer",
