...
import { ArrowLeft, BookOpen, FileQuestion, ChevronDown, ChevronUp } from "lucide-react";
...
interface CategoryGuidesListProps {
  category: Category;
  guides: Guide[];
...
const CategoryGuidesList: React.FC<CategoryGuidesListProps> = ({
  category,
  guides,
  questionsGrouped,
  readingsGrouped,
  ...
  return (
    <div>
      ...
      <CategoryHero
        ...
        activeGuidesCount={guides.filter(g => g.hasContent).length}
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                {relatedCategories.map((relatedCategory) => (
                  <Link
                    ...
                    href={`/guides/categories/${relatedCategory.id}`}
                    className="border-border hover:border-primary bg-background group block rounded-lg border p-4 transition-all hover:shadow-md"
                  >
                    <h3 className="group-hover:text-primary font-semibold transition-colors text-sm">
      ...
      <div className="sr-only" aria-hidden="true">
        <h2>Complete {category.name} Planning Guide Collection</h2>
        <p>
          {totalQuestionsCount} planning questions and {totalReadingsCount} expert readings across {guides.length} guides.
        </p>
