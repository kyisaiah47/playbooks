...
const formSchema = z.object({
	...
	returnToWorkPlan: z.enum([
		"full-time",
		"part-time",
		"work-from-home",
		"career-break",
	]),
	childcareArrangements: z.enum([
		"daycare",
		"nanny",
		"family",
		"sahp",
...
export function BabyPlanningSetupWizard({
	...
	const [homePreparation, setHomePreparation] = useState<string[]>(
		initialData?.homePreparation || []
	);
	const [familySupport, setFamilySupport] = useState<string[]>(
		initialData?.familySupport || []
	);
	const [friendsSupport, setFriendsSupport] = useState<string[]>(
		initialData?.friendsSupport || []
	);
	const [professionalSupport, setProfessionalSupport] = useState<string[]>(
		initialData?.professionalSupport || []
	);
	const totalSteps = 8;
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input
											placeholder="e.g., Emma, Noah, or leave blank"
											{...field}
										/>
									</FormControl>
									<FormMessage />
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0">
									...
									<div className="space-y-1 leading-none">
										<FormLabel>This is my first baby</FormLabel>
									</div>
								</FormItem>
							)}
						/>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input
											placeholder="e.g., Woodland Animals, Space, Floral"
											{...field}
										/>
									</FormControl>
									<FormMessage />
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setHomePreparation(
														homePreparation.filter((t) => t !== task)
													);
												}
											}}
										/>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0">
									...
									<div className="space-y-1 leading-none">
										<FormLabel>
											Baby-proofing will be needed (not first-time parents or
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setFamilySupport(
														familySupport.filter((p) => p !== person)
													);
												}
											}}
										/>
										<label
											htmlFor={`family-${person}`}
											className="text-sm font-normal"
										>
											{person}
										</label>
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setFriendsSupport(
														friendsSupport.filter((p) => p !== person)
													);
												}
											}}
										/>
										<label
											htmlFor={`friends-${person}`}
											className="text-sm font-normal"
										>
											{person}
										</label>
								...
								{supportOptions.slice(4).map((person) => (
									...
									>
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													setProfessionalSupport([
														...professionalSupport,
														person,
													]);
												} else {
													setProfessionalSupport(
														professionalSupport.filter((p) => p !== person)
													);
												}
											}}
										/>
										<label
											htmlFor={`professional-${person}`}
											className="text-sm font-normal"
										>
											{person}
										</label>
								...
								<FormItem>
									...
									>
										...
										<SelectContent>
											...
											<SelectItem value="career-break">
												Take extended career break
											</SelectItem>
										</SelectContent>
									</Select>
									<FormMessage />
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0">
										...
										<div className="space-y-1 leading-none">
											<FormLabel>Planning a baby shower</FormLabel>
										</div>
									</FormItem>
								)}
							/>
