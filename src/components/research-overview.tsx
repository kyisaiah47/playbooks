...
import {
	useResearch,
	getResearchDisplayData,
} from "@/contexts/research-context";
import {
	BookOpen,
	Users,
	Clock,
...
export function ResearchOverview() {
	...
	const quickStats = [
		...
		{
			label: "Team Size",
			value: `${researchData.teamSize} member${
				researchData.teamSize !== 1 ? "s" : ""
			}`,
			icon: Users,
			color: "text-foreground",
		},
			...
			<Card>
				...
				<CardContent className="space-y-4">
					<div className="space-y-2">
						...
						<Progress
							value={displayData.progressPercentage}
							className="h-3"
						/>
					</div>
					<div className="grid grid-cols-2 gap-4 text-sm">
						<div className="flex justify-between">
							...
							<span className="font-medium">
								{displayData.daysUntilDeadline}
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">Expected Completion</span>
							<span className="font-medium">
								{displayData.endDate.toLocaleDateString()}
							</span>
						</div>
					</div>
				</CardContent>
