...
const formSchema = z.object({
	...
	careerGoals: z
		.string()
		.min(10, "Career goals should be at least 10 characters"),
	preferredSchoolTypes: z.array(
		z.enum([
			"public",
			"private",
			"liberal-arts",
	...
	maxApplications: z
		.number()
		.min(1, "Must plan at least 1 application")
		.max(30, "Maximum 30 applications recommended"),
	...
	personalStatement: z
		.string()
		.min(20, "Personal statement should be at least 20 characters"),
...
export function CollegePlanningSetupWizard({
	isOpen,
	onClose,
	onComplete,
	initialData,
}: CollegePlanningSetupWizardProps) {
	const [currentStep, setCurrentStep] = useState(1);
	const [alternativeMajors, setAlternativeMajors] = useState<string[]>(
		initialData?.alternativeMajors || []
	);
	const [academicInterests, setAcademicInterests] = useState<string[]>(
		initialData?.academicInterests || []
	);
	...
	const [apCoursesTaken, setApCoursesTaken] = useState<string[]>(
		initialData?.standardizedTests.ap.coursesTaken || []
	);
	const [apCoursesPlanned, setApCoursesPlanned] = useState<string[]>(
		initialData?.standardizedTests.ap.coursesPlanned || []
	);
	const [essayTopics, setEssayTopics] = useState<string[]>(
		initialData?.essayTopics || []
	);
	const [legacyStatus, setLegacyStatus] = useState<string[]>(
		initialData?.legacyStatus || []
	);
	const [specialTalents, setSpecialTalents] = useState<string[]>(
		initialData?.specialTalents || []
	);
	const totalSteps = 8;
	...
	const form = useForm<z.infer<typeof formSchema>>({
		...
		defaultValues: {
			...
			graduationYear:
				initialData?.graduationYear || new Date().getFullYear() + 1,
			...
			expectedFamilyContribution:
				initialData?.expectedFamilyContribution || 10000,
					...
					<div className="space-y-4">
						...
						<FormField
							...
							render={() => (
								<FormItem>
									<FormLabel>
										Preferred School Types (Select all that apply)
									</FormLabel>
						...
						<FormField
							...
							render={() => (
								<FormItem>
									<FormLabel>
										Preferred School Sizes (Select all that apply)
									</FormLabel>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>
										How much can your family contribute annually toward college
										costs?
									</FormDescription>
									<FormMessage />
								</FormItem>
							...
							render={({ field }) => (
								<FormItem>
									...
									>
										...
										<SelectContent>
											<SelectItem value="virtual-only">
												Virtual visits only
											</SelectItem>
											<SelectItem value="in-person-only">
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0">
										...
										<FormLabel>I am an international student</FormLabel>
									</FormItem>
								)}
							/>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0">
										...
										<FormLabel>I have access to a college counselor</FormLabel>
									</FormItem>
								)}
							/>
									...
									>
										...
										<SelectContent>
											<SelectItem value="high">
												High - Very involved in the process
											</SelectItem>
											<SelectItem value="medium">
												Medium - Some guidance and support
											</SelectItem>
