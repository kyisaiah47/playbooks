...
const formSchema = z.object({
	...
	minSquareFeet: z
		.number()
		.min(300, "Minimum square feet must be at least 300"),
	maxSquareFeet: z
		.number()
		.min(500, "Maximum square feet must be at least 500"),
	...
	monthlyIncomeAfterTax: z
		.number()
		.min(2000, "Monthly income must be at least $2,000"),
...
export function HomeBuyingSetupWizard({
	...
	const [neighborhoodPreferences, setNeighborhoodPreferences] = useState<
		string[]
	>(initialData?.neighborhoodPreferences || []);
	const [commutePreferences, setCommutePreferences] = useState<string[]>(
		initialData?.commutePreferences || []
	);
	const [lenderPreferences, setLenderPreferences] = useState<string[]>(
		initialData?.lenderPreferences || []
	);
	const totalSteps = 8;
	...
	const form = useForm<z.infer<typeof formSchema>>({
		...
		defaultValues: {
			...
			realEstateAgent: initialData?.realEstateAgent || {
				name: "",
				phone: "",
				email: "",
				company: "",
			...
			homeInspector: initialData?.homeInspector || {
				name: "",
				phone: "",
				email: "",
				company: "",
			},
			attorneyTitle: initialData?.attorneyTitle || {
				name: "",
				phone: "",
				email: "",
				company: "",
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setMustHaveFeatures(
														mustHaveFeatures.filter((f) => f !== feature)
													);
												}
											}}
										/>
										<label
											htmlFor={`must-${feature}`}
								...
								{propertyFeatures.map((feature) => (
									...
									>
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													setNiceToHaveFeatures([
														...niceToHaveFeatures,
														feature,
													]);
												} else {
													setNiceToHaveFeatures(
														niceToHaveFeatures.filter((f) => f !== feature)
													);
												}
											}}
										/>
										<label
											htmlFor={`nice-${feature}`}
											className="text-sm font-medium"
										>
											{feature}
										</label>
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
									...
									<div className="space-y-1 leading-none">
										<FormLabel>First-time home buyer</FormLabel>
										<FormDescription>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input
											placeholder="e.g., Palo Alto Unified"
											{...field}
										/>
									</FormControl>
									<FormMessage />
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>
										How important is it to walk to amenities? (0 = not
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														placeholder="Attorney/Title Rep Name"
														{...field}
													/>
												</FormControl>
												<FormMessage />
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														placeholder="Law Firm/Title Company"
														{...field}
													/>
												</FormControl>
												<FormMessage />
