...
const defaultGearItems: GearItem[] = [
	...
	// Diaper & Bath
	{
		...
		category: "diaper-bath",
		priority: "essential",
		estimatedCost: 100,
		purchased: false,
		notes: "Start with variety pack",
	},
	{
		id: "16",
		name: "Baby Wipes",
	...
	// Clothing
	{
		...
		estimatedCost: 50,
		purchased: false,
		notes: "Newborn and 0-3 months",
	},
	{
		id: "24",
		name: "Sleepers/Pajamas (6-8)",
		category: "clothing",
		priority: "essential",
		estimatedCost: 60,
		purchased: false,
	...
	},
	{
		...
		estimatedCost: 25,
		purchased: false,
		notes: "2 sizes just in case",
	},
	{
		id: "26",
		name: "Socks/Booties (6 pairs)",
		category: "clothing",
		priority: "essential",
		estimatedCost: 20,
		purchased: false,
	...
	// Mobility & Travel
	{
		...
		estimatedCost: 200,
		purchased: false,
		notes: "Must have before leaving hospital",
	},
	{
		id: "30",
		name: "Stroller",
...
export function BabyGearChecklist() {
	...
	const getProgressStats = () => {
		const essentialItems = gearItems.filter(
		...
		const totalCost = gearItems.reduce(
			(sum, item) => sum + item.estimatedCost,
			0
		);
		const spentCost = gearItems
			.filter((item) => item.purchased)
			.reduce((sum, item) => sum + item.estimatedCost, 0);

		return {
			essentialProgress: Math.round(
				(purchasedEssential.length / essentialItems.length) * 100
			),
			totalProgress: Math.round(
				(totalPurchased.length / gearItems.length) * 100
			),
			totalCost,
			spentCost,
		...
		<div className="space-y-6">
			...
			<div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{stats.purchasedEssentialCount}/{stats.essentialCount}
						</div>
						<div className="flex items-center space-x-2 mt-2">
							<Progress
								value={stats.essentialProgress}
								className="flex-1 h-2"
							/>
							<span className="text-xs text-muted-foreground">
								{stats.essentialProgress}%
							</span>
						</div>
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{stats.purchasedCount}/{stats.totalCount}
						</div>
						<div className="flex items-center space-x-2 mt-2">
							<Progress
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							${stats.spentCost.toLocaleString()}
						</div>
						<p className="text-xs text-muted-foreground mt-1">
							...
							return (
								<Card key={category.id}>
									<CardHeader>
										...
										<Progress
											value={Math.round(
												(purchasedCount / categoryItems.length) * 100
											)}
											className="h-2"
										/>
									</CardHeader>
