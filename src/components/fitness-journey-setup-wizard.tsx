...
const formSchema = z.object({
	...
	pregnancyStatus: z
		.enum(["not-pregnant", "pregnant", "postpartum"])
		.optional(),
	...
	motivationalStatement: z
		.string()
		.min(10, "Statement should be at least 10 characters"),
});
...
export function FitnessJourneySetupWizard({
	isOpen,
	onClose,
	onComplete,
	initialData,
}: FitnessJourneySetupWizardProps) {
	const [currentStep, setCurrentStep] = useState(1);
	const [fitnessGoals, setFitnessGoals] = useState<string[]>(
		initialData?.fitnessGoals || []
	);
	const [workoutPrefs, setWorkoutPrefs] = useState<string[]>(
		initialData?.workoutPreference || []
	);
	const [availableSlots, setAvailableSlots] = useState<string[]>(
		initialData?.availableTimeSlots || []
	);
	const [restDays, setRestDays] = useState<string[]>(
		initialData?.restDays || []
	);
	const [nutritionGoals, setNutritionGoals] = useState<string[]>(
		initialData?.nutritionGoals || []
	);
	const [motivations, setMotivations] = useState<string[]>(
		initialData?.motivationFactors || []
	);
	const totalSteps = 8;
			...
			case 3:
				return (
					<div className="space-y-4">
						...
						<div>
							<FormLabel>
								Workout Types (Select all that interest you)
							</FormLabel>
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setWorkoutPrefs(
														workoutPrefs.filter((w) => w !== workout)
													);
												}
											}}
										/>
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											...
											<FormDescription>I have access to a gym</FormDescription>
										</div>
									</FormItem>
								)}
							/>
										...
										<Checkbox
											...
											onCheckedChange={(checked) => {
												if (checked) {
													...
													setNutritionGoals(
														nutritionGoals.filter((g) => g !== goal)
													);
												}
											}}
										/>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormDescription>Leave blank if unsure</FormDescription>
										<FormMessage />
									</FormItem>
								)}
							/>
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											...
											<FormDescription>I work with a trainer</FormDescription>
										</div>
									</FormItem>
								)}
							/>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											<FormLabel>Nutritionist</FormLabel>
											<FormDescription>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											<FormLabel>Medical Supervision</FormLabel>
											<FormDescription>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											<FormLabel>Workout Buddy</FormLabel>
											<FormDescription>
				...
				return (
					<div className="space-y-4">
						<div className="text-center mb-6">
							...
							<p className="text-sm text-muted-foreground">
								Write a personal statement about your fitness goals and
								motivation
							</p>
						</div>
