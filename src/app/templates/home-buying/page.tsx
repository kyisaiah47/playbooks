...
import {
	useHomeBuying,
	getHomeBuyingDisplayData,
} from "@/contexts/home-buying-context";
import {
	Breadcrumb,
	BreadcrumbItem,
	BreadcrumbList,
...
function HomeBuyingTemplatePage() {
	...
	const { homeBuyingData, isWizardOpen, setWizardOpen, updateHomeBuyingData } =
		useHomeBuying();
	...
	const resetAllData = () => {
		if (
			window.confirm(
				"Are you sure you want to reset all home buying data? This action cannot be undone."
			)
		) {
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground">
					Manage your home buying preferences and data
				</p>
			</div>
				...
				<div className="rounded-lg border p-6">
					...
					<div className="grid md:grid-cols-2 gap-4 text-sm">
						<div>
							...
							<p className="text-muted-foreground">
								{homeBuyingData.targetLocation || "Not set"}
							</p>
						</div>
						<div>
							<span className="font-medium">Budget Range:</span>
							<p className="text-muted-foreground">
								{homeBuyingData.currency}{" "}
								{homeBuyingData.minBudget.toLocaleString()} -{" "}
								{homeBuyingData.maxBudget.toLocaleString()}
							</p>
						</div>
						<div>
							<span className="font-medium">Property Type:</span>
							<p className="text-muted-foreground">
								{homeBuyingData.propertyType
									.replace("-", " ")
						...
						<div>
							...
							<p className="text-muted-foreground">
								{homeBuyingData.bedrooms} bed, {homeBuyingData.bathrooms} bath
							</p>
						</div>
						<div>
							<span className="font-medium">First Time Buyer:</span>
							<p className="text-muted-foreground">
								{homeBuyingData.firstTimeBuyer ? "Yes" : "No"}
							</p>
						</div>
						<div>
							<span className="font-medium">Timeline:</span>
							<p className="text-muted-foreground">
								{homeBuyingData.searchTimeline
									.replace("-", " ")
									.replace(/\b\w/g, (l) => l.toUpperCase())}
							</p>
						</div>
	...
	const HomeBuyingHelp = () => (
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground">
					Tips and guidance for your home buying journey
				</p>
			</div>
					...
					<div className="space-y-4">
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Use the setup wizard to configure your home buying preferences
								and budget.
							</p>
						</div>
						<div>
						...
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Use the property search tool to track homes you&apos;re
								interested in and schedule viewings.
							</p>
						</div>
						<div>
							<h4 className="font-medium mb-2">4. Build Your Team</h4>
							<p className="text-sm text-muted-foreground">
								Use the agent tracker to manage relationships with real estate
								professionals.
							</p>
						</div>
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Use the moving planner to organize your relocation timeline and
								tasks.
							</p>
						</div>
					</div>
